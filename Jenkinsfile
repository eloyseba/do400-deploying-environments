...
'''
sh '''
./mvnw package -DskipTests -Dquarkus.container-image.build=true -Dquarkus.container-image.registry=quay.io -Dquarkus.container-image.group=$QUAY_USR -Dquarkus.container-image.name=do400-deploying-environments -Dquarkus.container-image.username=$QUAY_USR -Dquarkus.container-image.password="$QUAY_PSW" -Dquarkus.container-image.tag=build-${BUILD_NUMBER} -Dquarkus.container-image.push=true
'''
}
}
stage('Deploy - Stage') {
